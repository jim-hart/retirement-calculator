...
def test_written_percentages_converted_to_float(field, mock_user_data):
    user = User.model_validate(mock_user_data)
    assert getattr(user, field) == mock_user_data[field] / 100


def test_retirement_age_must_be_greater_than_current_age(mock_user_data):
...
def test_life_expectancy_must_be_greater_than_retirement_age(mock_user_data):
    mock_user_data["life_expectancy"] = mock_user_data["retirement_age"]
    with pytest.raises(ValidationError):
        User.model_validate(mock_user_data)


def test_current_age_matches_expected_value(mock_user_data):
    """
    Verify User.current_age is correctly calculated using User.date_of_birth
    """
    expected_age = 50
    date_of_birth = date.today() - timedelta(days=(365 * expected_age) + 10)
    mock_user_data["date_of_birth"] = date_of_birth.isoformat()
    user = User.model_validate(mock_user_data)
    assert user.current_age == expected_age


def test_calculated_years_to_retirement_matches_expected_value(mock_user_data):
    user = User.model_validate(mock_user_data)
    expected_years = mock_user_data["retirement_age"] - user.current_age
    assert user.years_to_retirement == expected_years


def test_calculated_years_in_retirement_matches_expected_value(mock_user_data):
    user = User.model_validate(mock_user_data)
    expected_years = (
        mock_user_data["life_expectancy"] - mock_user_data["retirement_age"]
    )
